...
::   IDM 激活脚本 (IAS)
::
::   网站主页: https://github.com/WindowsAddict/IDM-Activation-Script
...
::  若要激活, 请使用 "/act" 参数运行脚本, 或在下一行中将 0 更改为 1
set _activate=0

::  要冻结 30 天试用期, 请使用 "/frz" 参数运行脚本, 或在下一行中将 0 更改为 1
set _freeze=0

::  若要重置激活和试用, 请使用 "/res" 参数运行脚本, 或在下一行中将 0 更改为 1
set _reset=0

::  如果更改了上述行中的值或使用参数, 则脚本将在无人值守模式下运行

::========================================================================================================================================

::  设置路径变量, 如果在系统中配置错误, 会有帮助
...
:: 如果脚本是由 x64 位 Windows 上的 x86 进程启动的, 请使用 x64 进程重新启动脚本
:: 或者使用 ARM64 进程 (如果它是由 ARM64 Windows 上的 x86/ARM32 进程启动的) 
...
:: 如果脚本是由 ARM64 Windows 上的 x64 进程启动的, 请使用 ARM32 进程重新启动脚本
...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 出现错误: 脚本存在 LF 行结尾问题, 或者脚本末尾缺少空行.
echo:
...
set "nceline=echo: &echo ==== 出现错误 ==== &echo:"
set "eline=echo: &call :_color %Red% "==== 出现错误 ====" &echo:"
...
echo 此项目仅支持 Windows 7/8/8.1/10/11 及其相应的服务器版本.
goto done2
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 此系统无法找到 powershell.exe.
goto done2
)
...
echo:
echo 查看帮助页. %mas%idm-activation-script.html#Troubleshoot
goto done2
)

::========================================================================================================================================

::  将脚本提升为管理员, 并传递参数和防止循环
...
::  禁用快速编辑并从 conhost.exe 启动以避免终端应用程序
...
echo [0] 仍然继续
echo:
call :_color %_Green% "在键盘中输入菜单选项 [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
echo:
echo 在 %SystemRoot%\Temp 中创建 CLSID 注册表键值备份
...
echo %line%
echo:
echo      激活对某些用户不起作用, IDM 可能会显示假冒序列号的弹出窗口.
echo:
call :_color2 %_White% "     " %_Green% "推荐使用冻结试用期选项代替."
echo %line%
echo:
choice /C:19 /N /M ">    [1] 返回上一页 [9] 激活 : "
...
echo 你可以从此下载  https://www.internetdownloadmanager.com/download.html
goto done
)

:: 使用 internetdownloadmanager.com ping 和端口 80 测试进行 Internet 检查
...
echo:
call :_color %Gray% "如果出现假冒序列号窗口, 请使用 "冻结试用" 选项."
) else (
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "正在 $regPath 中搜索 IDM CLSID 注册表键值"
	Write-Host
    ...
    foreach ($lockedKey in $lockedKeys) {
        ...
        Write-Output "$leafValue - 找到已锁定键值"
    }
    ...
    foreach ($key in $filteredKeys) {
        ...
        if (($defaultValue -match "^\d+$") -and ($key.SubKeyCount -eq 0)) {
            ...
            Write-Output "$($key.PSChildName) - 在默认中找到数字, 无子键"
            continue
        }
        if (($defaultValue -match "\+|=") -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - 在默认中找到 + 或 = , 无子键"
            continue
        }
...
if ($finalValues -ne $null) {
    Write-Host
    if ($lockKey -ne $null) {
        Write-Host "正在锁定 IDM CLSID 注册表键值..."
    }
    if ($deleteKey -ne $null) {
        Write-Host "正在删除 IDM CLSID 注册表键值..."
    }
    Write-Host
} else {
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM 键值计数超过 20. 现在删除它们而不是锁定..."
	Write-Host
}
